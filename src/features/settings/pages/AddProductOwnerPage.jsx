...
export default function AddProductOwnerPage() {
  const [formData, setFormData] = useState(initialFormData)
  const [errors, setErrors] = useState({})
  const [successMessage, setSuccessMessage] = useState('')
  const { mutate, isPending } = useAddTenantProductOwner()
  ...
  const handleSubmit = (event) => {
    ...
    mutate(payload, {
      onSuccess: () => {
        ...
        setSuccessMessage(`New Product Owner account created successfully for ${payload.email}!`)
      },
    })
  }
    ...
    <div className="flex flex-col gap-6 max-w-3xl">
      ...
      <div className="flex flex-col gap-1 border-b border-slate-200/80 pb-5">
        ...
        <p className="text-sm text-slate-500 mt-0.5">
          Add a new Product Owner to your organization (tenant) with administrative access to workspace features.
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-6">
        <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
          ...
          <div className="pt-2 flex justify-end">
            ...
            >
              <UserPlus size={16} />
              <span>{isPending ? 'Adding Product Owner...' : 'Add Product Owner'}</span>
            </Button>
          </div>
        </form>
      </div>
