...
const EVENT_NAME = 'feedinsight:triage_seen_feedbacks_updated'

function getStorageKey(userId) {
  return `feedinsight_triage_seen_feedbacks_${userId || 'po'}`
}
...
function saveSeenMap(userId, map) {
  try {
    ...
    console.error('Failed to save triage seen feedbacks to localStorage:', err)
  }
}

/**
 * Hook to track unseen triage feedback submissions for Product Owner.
 * @param {Array} items List of triage feedback items
 */
export function useUnseenTriageFeedbacks(items = []) {
  const { user } = useAuth()
  ...
  const totalUnseenCount = useMemo(() => {
    ...
    return items.filter((item) => item?.id && !seenMap[item.id]).length
  }, [items, seenMap])
