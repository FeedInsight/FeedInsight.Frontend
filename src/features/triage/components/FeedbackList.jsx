...
export default function FeedbackList({
  ...
  const { rawItems, totalItems, hasNextPage, hasPreviousPage } = useMemo(() => {
    if (!feedbacksData)
      return { rawItems: [], totalItems: 0, hasNextPage: false, hasPreviousPage: false }
    ...
    const hasNext =
      feedbacksData?.hasNextPage ??
      feedbacksData?.pagination?.hasNextPage ??
      page * pageSize < total
    const hasPrev =
      feedbacksData?.hasPreviousPage ?? feedbacksData?.pagination?.hasPreviousPage ?? page > 1
  ...
  const sortedItems = useMemo(() => {
    return [...rawItems].sort((a, b) => {
      ...
      const aCount =
        ...
        (Array.isArray(aTasks)
          ? aTasks.length
          : Array.isArray(aTasks?.items)
            ? aTasks.items.length
            : 0)
      const bCount =
        b.tasksCount ??
        b.extractedTasksCount ??
        (Array.isArray(bTasks)
          ? bTasks.length
          : Array.isArray(bTasks?.items)
            ? bTasks.items.length
            : 0)
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Header & Search Bar */}
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs dark:bg-slate-900 dark:border-slate-700">
        <div className="flex items-center gap-2.5">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-indigo-50 text-indigo-600 border border-indigo-100/80 shadow-2xs dark:bg-indigo-950 dark:text-indigo-300 dark:border-indigo-800">
            <Inbox size={19} />
          </div>
          <div>
        ...
        <div className="relative w-full sm:w-60">
          ...
          <Input
            ...
            className="w-full pl-8 text-xs bg-slate-50/70 border-slate-200 focus:bg-white rounded-xl dark:bg-slate-800 dark:border-slate-700 dark:text-slate-100 dark:focus:bg-slate-800"
          />
        </div>
      </div>
      ...
      {/* Content List */}
      {isLoading ? (
        <div className="flex py-20 justify-center items-center gap-2.5 text-slate-500 text-xs bg-white rounded-2xl border border-slate-200/80 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-400">
          <Spinner size={24} className="text-brand-600" />
          <span>Loading triage inbox...</span>
        </div>
      ) : sortedItems.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-8 text-center shadow-xs dark:border-slate-700 dark:bg-slate-900">
          <EmptyState
            icon={Inbox}
        ...
        <div className="flex flex-col gap-4">
          <div
            className={
              isCompact
                ? 'flex flex-col gap-3 max-h-[calc(100vh-280px)] overflow-y-auto p-1'
                : 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[calc(100vh-280px)] overflow-y-auto p-1'
            }
          >
