...
export default function ChatInput({ onSend, isSending, showSuggestions = true }) {
  ...
  return (
    <div className="flex flex-col gap-2 border-t border-slate-200/90 bg-white p-3 sm:p-4">
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1.5 scrollbar-thin">
          ...
          {PROMPT_SUGGESTIONS.map((prompt, idx) => (
            <button
              ...
              className="shrink-0 rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-medium text-slate-700 hover:border-brand-300 hover:bg-brand-50 hover:text-brand-700 transition-colors disabled:opacity-50"
            >
              {prompt}
            </button>
          ))}
